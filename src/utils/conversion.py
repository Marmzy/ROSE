...
def check_gff(
    ...
    Raises:
        ValueError: ValueError if seqid column's values don't start with 'chr',
                    nor are Human NCBI refseq accesion numbers
    """
    ...
    # If .gff3 seqid column values do not start with "chr", see if they are
    # Human NCBI refseq accession numbers and replace them
...
def gtf_to_gff3(
    ...
    Args:
        ...
        output (str): Output .gff3 file path
        full (bool, optional): Boolean to fully convert the .gtf atttributes
                               to .gff3 format. Defaults to False.
    """

    # Initialising variables
    gtf_attributes = [
        "gene_id", "db_xref", "gbkey", "gene", "gene_biotype", "transcript_id",
        "model_evidence", "product", "exon_number", "protein_id", "anticodon",
        "inference", "note", "exception", "transl_except", "pseudo", "partial"
    ]
    chosen = gtf_attributes if full else ["gene_id", "transcript_id"]
    ...
    df.iloc[:, 8] = [
        ";".join([
            a
            for a in [search(attr, annot) for attr in chosen]
            if a is not None
        ])
        for annot in df.iloc[:, 8].values
    ]
...
def search(
    ...
    # Check if attribute is present for given entry, and if it is,
    # return it's value
    if re.search(f'{attr} "', entry):
        return "{}={}".format(
            attr, re.findall(rf"(?<={attr} \")[^\"]+", entry)[0]
        )
